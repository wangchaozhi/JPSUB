...
│  └─ cli.py            命令行入口
├─ shell/src-tauri/     Tauri 壳:拉起 sidecar、注入端口、凭据存取
...
- 在线模式:`engine=online`,支持 OpenAI-compatible Chat Completions 和 DeepL。桌面端可把 API key 保存到 Windows 凭据管理器;引擎也支持从运行时参数或环境变量读取,不要写入仓库或配置文件。
- 运行时环境变量:
...
CI 会在 push/PR 时跑测试和构建;推送 tag 会创建 GitHub Release,并上传 UI、sidecar、安装包和日志。示例:
